...
public class LocationManager extends SubsystemBase{
    ...
    public void loop(){
        ...
        if (Inputs.operatorTrigger && !Inputs.masterEndgameArm){
            //turnRobotToTarget(x, y);
            double[] pointF = returnMovingTarget();
            turnRobotToTarget(pointF[0], pointF[1]);
        ...
        /*if (m_extraControl.getRawButtonPressed(10)) {
            l += 5;
        } else if (m_extraControl.getRawButtonPressed(9)) {
            l -= 5;
        }

        if (m_extraControl.getRawButtonPressed(12)) {
            l += 1;
        } else if (m_extraControl.getRawButtonPressed(11)) {
            l -= 1;
        }*/
        ...
        /*turnTurretToTarget(x, y);
        setShooterPowerToTarget(x, y);
        setHoodAngleToTarget(x, y);*/
    ...
    public double returnAngleToTarget(double robotX, double robotY) {
        //return Math.atan(robotY / robotX) + Math.PI / 2;
        return Math.atan(robotX / robotY);// + Math.PI / 2;
    }

    public void turnRobotToTarget(double robotX, double robotY) {
        /*double radian = returnAngleToTarget(robotX, robotY);
        double middleTurretCompensation = (500 + l) * (Constants.ShooterConstants.kTurretFullRight - Constants.ShooterConstants.kTurretFullLeft) / 2;

        double angleDifference = radian * 180 / Math.PI - middleTurretCompensation;

        if (angleDifference > 0) {
            Inputs.rotationInputOverride = -0.1;
        } else {
            Inputs.rotationInputOverride = 0.1;
        }

        Inputs.rotationInputOverride = 0.0;
        SmartDashboard.putNumber("POSTMAN angle difference", angleDifference);

        if (!Inputs.overrideTurret) {
            Inputs.rotationOverride = true;
        } else {
        ...
        /*if (angleDifference < rightAngleBound){
            Inputs.rotationOverride = true;
            Inputs.rotationInputOverride = 0.1;
        } else if (angleDifference > leftAngleBound){
            Inputs.rotationOverride = true;
            Inputs.rotationInputOverride = -0.1;
        } else {
            Inputs.rotationOverride = false;
    ...
    private double convertPowerToDistance(double powerApplied) {
        /*double percentage = powerApplied * Math.abs(powerApplied);
        double distance = percentage * 0.05; */
