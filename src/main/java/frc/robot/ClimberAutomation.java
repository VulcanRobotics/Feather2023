...
public class ClimberAutomation extends MyStateMachine {
    ...
    public void switchSteps() {
        ...
        Constants.telemetry.saveDouble("CLIMB SM Step Timer", timStepTimer.get() );  // capture timer on ever step
        ...
        switch (iStep) {                            // switch statement.
            ...
            case 1:                               
                ...
                if( ClimbSubsystem.mLift1DownHasBeenReached == false ){     // we are not pulled down all the way
                    ...
                    // we will put other stuff here to drop a little using the 
                    // encode on the Lift 1 motor that was reset to 0.0 when we
                    // pulled all the way up. When we hit our mark we will declare thsi step complete.
                    // here eventually we will also set the brake when done.
                    iCompletedStepParts++;                                      // this part is done
                }

                /***************************************************************************
                 * This will push the hand out to the end of the arm. 
                 * We want to do this wil pulling up to save time later. 
                 * This is a very short distance so we are done quickly. 
                 ****************************************************************************/
                ...
                //ZACH UPDATES - DART SIGN FLIPPED (4/22/22) so changed > into < & vice versa, not necessary here
                ...
                if( ClimbSubsystem.mLift1DownHasBeenReached == false && ClimbSubsystem.getDartPosition() >= Constants.ClimberConstants.kStep1DartUpToBarPosit + 0.1) {// The robot is lifted up to the bar. was >=
                    //Inputs.climberDartPowerOverride = 0.0;
                ...
                //Inputs.climberDartPowerOverride = 0.0;
                    ...
                    //Inputs.climberDartPowerOverride = 0.0;
                    //Inputs.climberDartPowerOverride =  .3;
            ...
            case 2:             
                ...
                //ZACH UPDATES - DART SIGN FLIPPED (4/22/22) so changed >= into <=
                ...
                if( ClimbSubsystem.getDartPosition() <= Constants.ClimberConstants.kStep2DartSwingUpRobotPosit){    // make sure we are up to bar
                    ...
                    Constants.telemetry.putString("CLIMB SM Desc", status );
                    break;                              // go around agin unto Dart is at the bar position
                }else{
            ...
            case 4:             
                ...
                if( ClimbSubsystem.getLift2Position() <= Constants.ClimberConstants.kLift2FullInPosit){  // we are pulling up to robot
                    ...
                    Inputs.climberLift2Up = false;           // stop liftign up for now
                    iCompletedStepParts++;
